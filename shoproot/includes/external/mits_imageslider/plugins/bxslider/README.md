...
* Fix: Accessing bxslider's slider object inside its â€œonSliderLoadâ€ callback returns undefined (#475)
...
* `gulp` â€” Compile and optimize all files to `dist`
* `gulp styles` â€” Compile css assets only to `dist`
* `gulp scripts` â€” Compile js assets only to `dist`
* `gulp images` - Run lossless compression on all the images and copy to `dist`
* `gulp jshint` â€” Checks JS and JSON code for errors based on our .jshintrc settings
