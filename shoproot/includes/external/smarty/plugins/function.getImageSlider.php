...
function smarty_function_getImageSlider($params, &$smarty) {
  ...
  if ($slider_active == true) {
    ...
    if (xtc_db_num_rows($mits_slidergroups_query)) {
      ...
      if (isset($params['get_smarty_array']) && $params['get_smarty_array'] == true) {
        $mits_slider = MITS_get_imageslider($mits_slidergroups['imagesliders_group'], 'true');
        $smarty->assign($slidergroup, $mits_slider);
      } else {
